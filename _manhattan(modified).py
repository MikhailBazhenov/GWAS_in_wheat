# This is a modified _manhattan.py file from qmplot library
# https://github.com/ShujiaHuang/qmplot
# A couple of lines was changed here not to highlight the top SNPs with red color.
# To use it, place it instead of original _manhattan.py file in qmplot/qmplot/modules/ folder
...
def manhattanplot(data, chrom="#CHROM", pos="POS", pv="P", snp="ID", logp=True, ax=None,
    ...
    for seqid, group_data in data.groupby(by=chrom, sort=False):  # keep the raw order of chromosome
        ...
        for i, (site, p_value) in enumerate(zip(group_data[pos], group_data[pv])):
            ...
            y.append(y_value)
            c.append(color)
            # c.append(sign_marker_color if ((sign_marker_color is not 'None') and (p_value <= sign_marker_p)) else color)
    ...
    if is_annotate_topsnp:
        ...
        if sign_marker_color != 'None':
            ax.scatter(xs, ys, c='none', alpha=alpha, edgecolors=sign_marker_color, **kwargs)
